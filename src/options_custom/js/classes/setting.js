...
(function () {
    ...
    function fillInLabel (params) {
        ...
        if (
            ...
            params.text = params.text.get(params.name);
        }
    };
    ...
    var compareVersions = function (lhs, rhs) {
        if (lhs === rhs)
            return 0;
        ...
        for (var i = 0, l = rhsParts.length; i < l; i++) {
            var lhsElt = parseInt(lhsParts[i]);
            var rhsElt = parseInt(rhsParts[i]);

            if (lhsElt < rhsElt)
                return -1;
            else if (lhsElt > rhsElt)
                return 1;
        }
    ...
    Bundle.UpdateWidget = new Class({
        ...
        "createDOM": function () {
            ...
            this.statusSpan = new Element("span", {
                id: "updateStatus"
            });
        ...
        "setupDOM": function () {
            this.statusSpan.inject(this.container);
            (new Element("br", {})).inject(this.container);
        ...
        "installLatestVersion": function (evt) {
            ...
            var extensionUpdateText = null;
            var installerUpdateText = null;

            var isUpdatingExtension = true;
            var isUpdatingInstaller = true;
            var isUpdatingVersion = true;
            var didVersionChange = false;

            var previousVersionJsonTimestamp = null;
            chrome.runtime.sendMessage({type: "getSettings"}, function (settings) {
                previousVersionJsonTimestamp = settings.lastVersionJsonChange;
            });
            ...
            var refreshStatus = function () {
                ...
                if (categories.length === 0)
                    statusText = didVersionChange ? "Updated configuration from server." : "Done checking for updates.";
                else
                    statusText = "Checking for " + categories.join(", ") + " updates";
            ...
            chrome.runtime.sendMessage({type: "getSettings", force: true}, function (settings) {
                console.log("done", performance.now());
                isUpdatingVersion = false;
                didVersionChange = settings.lastVersionJsonChange !== previousVersionJsonTimestamp;
                refreshStatus();
            });
            ...
            sendInstallerCommand(evt.shiftKey ? "forceInstallUpdate" : "installUpdate", function (result, error) {
                ...
                switch (result.result) {
                    case "Updated":
                        self.extensionSpan.textContent = "Update successful. Installed v" + newVersion + ". Please restart Chrome or reload the extension.";
                        self.isRestartPending = true;
                        return;
                    case "NotUpdated":
                        self.extensionSpan.textContent = "No extension update found.";
                        break;
                    default:
        ...
        "refreshVersionText": function (installerVersion, installedVersion) {
            ...
            var updateDiff = 0;
            if (installerVersion)
                updateDiff = compareVersions(currentVersion, installedVersion);

            if (installerVersion) {
                
                this.extensionSpan.textContent += " (v " + installedVersion + 
                    (updateDiff < 0
                        ? " ready to install)"
                        : " is latest version)");
                this.installerSpan.textContent = "Installer v" + installerVersion;
            } else if (!installerVersion) {
                this.installerSpan.textContent = "Viramate Installer not available. If you installed from the Chrome Web Store please update from Chrome's Extensions page.";
            }
        ...
        "refresh": function () {
            ...
            sendInstallerCommand("getVersion", function (result, error) {
                if (result) {
                    ...
                    var isUpdateReady = compareVersions(currentVersion, result.extension) < 0;
