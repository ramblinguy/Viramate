...
var validHashesFromServer = null;
...
function doInitialize() {
    sendExtensionMessage({ type: "getValidHashes" }, function (hashes) {
        if (hashes)
            log("Got valid hashes from server", hashes);
        validHashesFromServer = hashes;
    });
    ...
    var secretToken;
    var onReadyToDetach;
    step1 = function (_ee, _secretToken, _onReadyToDetach) {
        ee = _ee;
        secretToken = _secretToken;
        onReadyToDetach = _onReadyToDetach;
        externalChannel = ee.channel;
        ee.evalInContext(jsFromClosure(_loadShaScript), step2);
    };
    step2 = function () {
        ee.evalInContext(jsFromClosure(_loadExternalScript), step3);
    };
    step3 = function () {
        log("Loaded sandbox scripts, initializing");
        finishChannelSetup(secretToken);
        secretToken = null;
        i18n.setLanguageGetter(function () {
            return currentSettings.language;
        });
        sendExtensionMessage({ type: "heartbeat" });
        sendExternalMessage({ type: "validHashes", hashes: validHashesFromServer });
        hasInitializedSandbox = true;
        tick();
        onReadyToDetach();
    };
    initExternalSandbox(step1, {
        bhstatic: getResourceUrl("bhstatic.json"),
        moduleIds: Object.keys(moduleIds)
    }, onMessageFromSandbox);
}
;
...
function showGamePopup(options) {
    ...
    if (!template) {
        logError("Could not find popup template");
        return;
    }
    var templateText = encodeURIComponent(template.textContent);
    var popupData = {
        "element_name": "pop",
        "title": options.title,
        "body": options.body,
...
function onKeyDown(evt) {
    ...
    if (isCombatPage(window.location.hash)) {
        ...
        if (!currentSettings.keyboardShortcuts3)
            return;
    }
    ...
    if (focused && (focused.className === encryptClassName("stamp-filter"))) {
        if (keyText === "ENTER") {
    ...
    if (combatState) {
    ...
    else if (window.location.hash.indexOf("arcarum2/") >= 0) {
        var buttons = document.querySelectorAll("div.prt-division-list div.btn-quest-list");
        var theButton = buttons[choiceNumber - 1];
        if (theButton) {
            pulseElement(theButton);
            generateClick(theButton);
            evt.preventDefault();
            evt.stopPropagation();
        }
    }
        ...
        if (keyText === "ARROWLEFT" || keyText === "ARROWRIGHT") {
            var pageNumber = parseInt(window.location.hash.substr(window.location.hash.lastIndexOf("/") + 1));
            var newPageNumber = pageNumber + ((keyText === "ARROWLEFT") ? 1 : -1);
            var btn = document.querySelector("li.btn-page-number[page=\"" + newPageNumber + "\"]");
            if (btn) {
                pulseElement(btn);
                generateClick(btn);
                evt.preventDefault();
...
function findOkButton() {
    ...
    findVisibleElementWithSelector("div.pop-stamina") ||
        ...
        // Using summons sends coords now
        findVisibleElementWithSelector("div.btn-summon-use"))
        return null;
    var result = findVisibleElementWithSelector("div.btn-usual-ok") ||
        ...
        // Runeknight Cast
        findVisibleElementWithSelector('div.pop-ability-mark div.prt-popup-footer div.btn-usual-text') ||
        findVisibleElementWithSelector('div.prt-division-list div.btn-move-division');
    return result;
}
;
...
function doClientAjax(...args) {
    ...
    if (isShutdown || (currentSettings && !currentSettings.allowGameAjax)) {
        log("Suppressed request for", url);
        if (callback)
            callback(null, isShutdown ? "compatibility shutdown" : "client ajax disabled", url);
        return;
    }
...
function onMessageFromSandbox(evt) {
    ...
    switch (evt.data.type) {
        case "moduleLoaded":
            validateModule(evt.data.id, evt.data.hash, evt.data.body);
            return;
        case "ajaxBegin":
        ...
        case "getExtensionVersion":
            ...
            return;
        case "getBookmarks":
            doGetBookmarks(evt.data.id, false);
            return;
        case "visitBookmark":
        ...
        case "tryUseSummon":
            ...
            return;
        case "trySelectCharacterByNumber":
            var selectResult = trySelectCharacterByNumber(evt.data.number, false, evt.data.id);
        ...
        case "tryJoinRaidDirectly":
            tryJoinRaidDirectly(evt.data.code, function (ok) {
                sendExternalMessage({
                    type: "socketResult",
                    id: evt.data.id,
                    result: ok
...
;
function doGetBookmarks(id, isRetry) {
    ...
    if (keys.length === 0) {
        ...
        if (isRetry) {
            ...
            window.setTimeout(function () { doGetBookmarks(id, true); }, 1000);
        }
    }
    else {
        sendExternalMessage({
...
;
function tryJoinRaidDirectly(raidId, onComplete) {
    var oc = function (x) {
        if (onComplete)
            onComplete(x);
        return x;
...
function tryJoinRaid(raidId, onComplete) {
    ...
    if (combatState && combatState.raidCode == raidId)
        return oc("already in this raid");
    if (!currentSettings.quickJoinFeature)
        return oc("feature disabled");
    visitRaidJoinPage(function (elt) {
        if (!elt)
            oc("page never loaded");
        else {
            var tab = document.querySelector("div.cnt-quest div.btn-tabs#tab-id");
            pulseElement(tab);
            generateClick(tab);
            log("filling in textbox");
            elt.value = raidId;
            elt.focus();
            oc("page loaded");
        }
...
function tryJoinCoOpRoom(roomId, onComplete) {
    ...
    if (!currentSettings.quickJoinFeature)
        return oc("feature disabled");
    visitCoOpJoinPage(function (elt) {
        if (!elt)
            oc("page never loaded");
        else {
            log("filling in textbox");
...
function parseUserStatus(status, uid) {
    // log("Captured status update from network");
...
function updatePerformanceHud(frameStats) {
    ...
    var performanceText = estimatedFps.toFixed(0) + "fps";
    if (frameStats.unskippedPercent < 100)
        performanceText += " " + frameStats.unskippedPercent.toFixed(0) + "%";
    performanceText += "\n";
    performanceText += "[" + minDelay.toFixed(0) + "-" + maxDelay.toFixed(0) + "] ms";
    performanceHudElement.textContent = performanceText;
