...
function onAjaxComplete(bundle) {
    ...
    if (url.indexOf("/quest/create_quest") >= 0) {
        ...
        sendExtensionMessage({
            type: "apiEvent:questCreated",
            uid: uid, url: url,
            questId: questId, raidId: raidId
        });
        // HACK
        ...
        if (responseData && responseData.option &&
            responseData.option.mydata_assets && responseData.option.mydata_assets.mydata) {
        ...
        try {
            onActionResult(url, responseData, uid, failed);
        }
        finally {
        ...
        try {
            ...
            if (!abilityId) {
                logError("Skill activation has no ability id", requestData);
                return;
            }
            if (failed) {
        ...
        if (responseData) {
            ...
            var twitter = responseData.twitter || {};
            sendExtensionMessage({
                type: "apiEvent:raidLoaded",
                uid: uid, url: url,
                raidId: twitter.raid_id,
                raidCode: twitter.battle_id,
                enemyId: twitter.enemy_id,
                viewerId: responseData.viewer_id
            });
        }
    ...
    else if (url.indexOf("/content/supporter") >= 0) {
        if (!responseData || !responseData.data)
            return;
        parseSupporterListData(responseData.data);
    }
    else {
...
function parseQuestResult(url, data, uid) {
    ...
    if (estimatedRpToNextRank <= 0)
        return;
    var rankUpText = "Next in " + estimatedRpToNextRank;
    var nextSpan = document.createElement("span");
    nextSpan.className = encryptClassName("next-rank-xp");
    nextSpan.textContent = rankUpText;
    rankup.appendChild(nextSpan);
}
;
...
;
function onActionResult(url, data, uid, failed) {
    ...
    sendExtensionMessage({
        type: "apiEvent:actionResult",
        succeeded: !failed,
        when: now, uid: uid, actionId: currentActionId,
        data: data, url: url,
        combatState: {
            party: combatState.party,
            enemies: combatState.enemies,
            raidId: combatState.raid_id,
            raidCode: combatState.raidCode,
            turn: combatState.turn,
            finish: combatState.finish,
        }
    });
