    ...
    checkItemFn = function (evt, onComplete) {
        ...
        if (checkItem) {
            if (data.targetUrl.endsWith("/0"))
                data.targetUrl = data.targetUrl.substr(0, data.targetUrl.length - 2);
        }
        else {
            if (!data.targetUrl.endsWith("/0"))
                data.targetUrl += "/0";
        ...
        doClientAjax(checkUrl, function (checkResult, error) {
            // This seems to be entirely unused now????
            if (error) {
                if (onComplete)
                    onComplete(false);
                return showBookmarkError(error);
            }
            var checkUrl2 = "/quest/check_quest_start/" + chapterId + "/1/" + data.questId;
            doClientAjax(checkUrl2, function (checkResult2, error2) {
                if (checkResult2 && (checkResult2.result === "ok") && !error2)
                    return checkStartFn(evt, onComplete);
                else {
                    if (onComplete)
                        onComplete(false);
                    return showBookmarkError("You cannot start this quest.");
                }
            });
        });
    };
...
var hardPlusRaidSubmenu = {
    "hardplus-tia": makeRaidBookmark(30501, 1),
    "hardplus-colo": makeRaidBookmark(30502, 1),
    "hardplus-levi": makeRaidBookmark(30503, 1),
    "hardplus-yugu": makeRaidBookmark(30504, 1),
    "hardplus-chev": makeRaidBookmark(30505, 1),
    "hardplus-cel": makeRaidBookmark(30506, 1)
};
var magna2RaidSubmenu = {
    "magna2-grimnir": makeTreasureRaidBookmark(30318, 1, 525),
    "magna2-shiva": makeTreasureRaidBookmark(30315, 1, 522),
    "magna2-europa": makeTreasureRaidBookmark(30316, 1, 523),
    "magna2-alexiel": makeTreasureRaidBookmark(30317, 1, 524),
    "magna2-metatron": makeTreasureRaidBookmark(30319, 1, 526),
    "magna2-avatar": makeTreasureRaidBookmark(30322, 1, 527)
};
var summonRaidSubmenu = {
...
var menuItems = {
    ...
    "hard-raids": hardRaidSubmenu,
    "hardplus-raids": hardPlusRaidSubmenu,
    "magna-raids": {
    ...
    "magna2-raids": magna2RaidSubmenu,
    "primal-raids": summonRaidSubmenu,
    "coop": {
        "coop": "/#coopraid",
...
var viramateMenuItems = {
    ...
    "combat-tracker": function () {
        var w = window.open("https://viramate.luminance.org/dps.html");
    },
...
function showViraButton() {
    ...
    populateSubmenu = function (container, items, isSubmenu, extraClassNames) {
        ...
        for (var k in items) {
            ...
            if ((v === guildWarSubmenu) &&
                ...
                continue;
            if ((v === hardRaidSubmenu) && !currentSettings.showHardRaids)
                continue;
            if ((v === hardPlusRaidSubmenu) && !currentSettings.showHardPlusRaids)
                continue;
            if ((v === magna2RaidSubmenu) && !currentSettings.showMagna2Raids)
                continue;
            if ((v === summonRaidSubmenu) && !currentSettings.showSummonRaids)
                continue;
    ...
    dateText.className = "viramate-config-date";
    try {
        if (currentSettings.lastVersionJsonChange) {
            var versionDate = new Date(Number(currentSettings.lastVersionJsonChange));
            dateText.textContent = "Config " + versionDate.toLocaleString("ja-JP");
            ul.appendChild(dateText);
        }
    }
    catch (exc) {
        log(exc);
    ...
    disabledText.textContent = "Some features disabled";
    if (currentSettings.remoteMessage)
        disabledText.textContent = currentSettings.remoteMessage;
    disabledText.className = "viramate-features-disabled";
    if ((currentSettings.remoteDisabledFeatures &&
        currentSettings.remoteDisabledFeatures.length) ||
        !currentSettings.allowGameAjax)
        ul.appendChild(disabledText);
...
;
function getRecentItems(panel, uid, callback, allowUpdate) {
    var msg = { type: "getItems", uid: uid, force: false };
    if (allowUpdate === false)
        msg.type = "getItemsNoUpdate";
